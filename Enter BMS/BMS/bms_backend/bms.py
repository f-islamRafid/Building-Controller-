...
def main():
    """Initializes the system and runs a unified login process."""
    print("\n")
    print("--- Building Management System (BMS) ---")
    system = BuildingSystem()
    
    # --- Interactive Setup Check (Runs only if config.json is missing) ---
    ...
    while True:
        ...
        if choice == '1':
            ...
            admin_role = system.authenticate_admin(email, password)
            if admin_role == "admin":
                print(f"\n>> Welcome, Administrator!")
                run_admin_menu(system)
                continue # Loop back to main menu after logout

            # 2. If not Admin, check if it is a Family User
            family_user = system.get_family_user(email, password)
            if family_user:
                print(f"\n>> Welcome, Resident of Flat {family_user.flat_no}!")
                run_family_menu(system, family_user)
                continue # Loop back to main menu after logout
